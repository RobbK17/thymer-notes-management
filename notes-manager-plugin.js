...
const NOTES_MANAGER_VERSION = '1.0.18';
...
class NotesManagerPanel {
    ...
    load() {
        ...
        this.plugin.ui.injectCSS(
            ...
            '.nm-bd-filter-pair{display:grid;grid-template-columns:1fr 1fr;column-gap:10px;row-gap:6px;grid-column:1/-1;align-items:stretch}' +
            '.nm-bd-filter-headrow{display:flex;width:100%;gap:8px;flex-wrap:wrap;box-sizing:border-box}' +
            '.nm-bd-filter-headrow--prop{justify-content:space-between;align-items:center}' +
            '.nm-bd-filter-headrow--title{justify-content:flex-start;align-items:flex-start}' +
...
<p class="nm-muted" style="margin-top:6px;font-size:12px">Saved as comma-separated collection names or GUIDs (same as before).</p>
    ...
    _buildBulkDeleteHTML() {
        ...
        const childrenOnlyEmpty = st.subPageFilter === 'children-only' && !st.records.some(r => r.hasParent);
        const parentHidden = st.subPageFilter !== 'match-parent' && !childrenOnlyEmpty ? ' hidden' : '';
        const noMatchParentChoices = st.subPageFilter === 'match-parent' && !st.parentOptions.length;
        let parentPlaceholder = 'Select parent...';
        if (noMatchParentChoices) parentPlaceholder = 'No parent to match in collection';
        else if (childrenOnlyEmpty) parentPlaceholder = 'No parent note(s)';
        const parentSelectOptsHtml = childrenOnlyEmpty ? '' : parentOpts;
        const parentDisabled = st.subPageFilter === 'match-parent'
            ? (noMatchParentChoices ? ' disabled' : '')
            : (childrenOnlyEmpty ? ' disabled' : ' disabled');
